			...
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="x_panel">
					...
					<div class="x_content">
						...
						<form action="{{route('procesocertificado', $id)}}" autocomplete="off" method="get">
							...
							<div class="form-group">
								<div class="row">
									...
									<label class="control-label" for="first-name">Certificado</label>
										<select name="tipoid"  id="" class="form-control" required>
											@foreach ($certificados as $tipo)
												@if ($certificadoObj != null)
													@if ($certificadoObj->certificadoid == $tipo->id)
														<option value="{{$certificadoObj->certificadoid}}" selected="selected">{{$tipo->descripcion}}</option>
													@else
								...
								<div class="row">
									<div class="col-md-6 col-sm-6 col-xs-12">
										...
										@if ($certificadoObj != null)
											<textarea class="text-area" name="obser" id="" cols="" rows="">{{$certificadoObj->observaciones}}</textarea>
										@else
