...
<div class="right_col" role="main">
  <div class="">
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>Subprocesos Planificados - <small>Cargue y Ordene los procesos</small></h2><br>
          ...
          <div class="x_content">
            <div class="row">
              <div class="">
                  ...
                  <table class="table table-striped jambo_table bulk_action">
                    <thead>
                      <tr class="headings">
                        ...
                        <th class="column-title">Kg/m MP</th>
                        <th class="column-title">Kilos</th>
                      </tr>
                    </thead>
                    <tbody>
                      @foreach ($versionesxorden as $version)
                        <tr class="even pointer">
                          <td>{{$version->orden}}</td>
                          <td>{{$version->cliente}}</td>
                          <td>{{$version->diamex}}</td>
                          <td>{{$version->esp}}</td>
                          <td>{{$version->diamein}}</td>
                          <td>{{$version->costura}}</td>
                          <td>{{$version->tratamiento}}</td>
                          <td>{{$version->kgm}}</td>
                          <td>{{$version->mts}}</td>
                          <td>{{$version->ext}}</td>
                          <td>{{$version->espesor}}</td>
                          <td>{{$version->lg}}</td>
                          <td>{{$version->kgmp}}</td>
                          <td>{{$version->kilos}}</td>
                        </tr>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <button type="button" class="btn btn-primary btn-sm" onclick="alertGuardado()">Guardar</button>
      ...
      $.toast({ 
          ...
          position : 'top-right'       
                    ...
                    <a href="{{url('buscarprocesoop', ['id'=>$id, 'type'=>'0'])}}" class="btn btn-primary btn-sm">Buscar Proceso</a>
                  </div>
                </div><br>
                <div class="x_title"></div>
                <div class="row">
                  <div id="loader"  style="display: none; margin:0 auto; text-align: center;"><img src="{{ asset ('build/images/loading.gif') }}" width="50" height="50">Eliminando Proceso</div>
                  <div id="loader2"  style="display: none; margin:0 auto; text-align: center;"><img src="{{ asset ('build/images/loading.gif') }}" width="50" height="50">Cargando Procesos...</div>
                  <div class="col-md-8 col-sm-8 col-xs-12">
                    <ol class='sub-proceso' id="insertproceso">
 ...
 <!-- INICIO SECCION MODAL -->
  ...
  <input type="hidden" id="procesos" value="{{$procesosplanificados}}">
  <input type="hidden" id="selectores" value="{{$res4}}">
  <input type="hidden" id="idorden" value="{{$id}}">
...
<script type="text/javascript">
  $(function(){
    ...
    var cotizacion = $('#idorden').val();
    var ejecutandose = JSON.parse($('#procesos').val());
    ...
    function deleteproceso(idpro, nropr, idcoti, remove){
      ...
      $.ajax({  
        ...
        data: {
          ...
          'esop': 1  //1 es orden de prod. / 0 es cotizacion
    ...
    $(document).on('click', 'li.lista-proceso', function(){
   ...
   $(document).on('click', '#volver', function(){
    ...
    function selectorestrefila(){
      ...
      function changeMatriz(este){
            $('#temp_direccion span').each(function(){
                $(this).hide();
            })
            var value = $(este).val();
            $('#data'+value).fadeIn();
    ...
    function calcRed(kgm){
        ...
        console.log('kgggggggmmmmmmmm',kgm)
        $("#mp-19-"+posicion).val(kgmt);

        //$("#mp-19-"+posicion).val(redondear((1-(kgmt/kgm))*100,2));
    }

    function calcEspe(){
        var d1 = parseFloat($("#mp-14-"+posicion).val(),2);
        var d2 = parseFloat($("#mp-15-"+posicion).val(),2);
        $("#mp-17-"+posicion).val((d1-d2)/2);
        var es = $("#mp-17-"+posicion).val();
        var kgmtc = redondear(((d1-es)*es)*0.0246,2);
